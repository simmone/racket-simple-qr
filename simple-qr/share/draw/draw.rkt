...
(provide (contract-out
          [draw (->*
                 (natural? natural? (hash/c (cons/c natural? natural?) string?) path-string? (or/c 'svg 'png) void?)
                 (#:foreground_color string? #:background_color string?)
                 )
                ]
          ))
...
(define (draw modules module_width points_map file_name output_type
  ...
  (cond
   ...
   [else
    (draw-png modules module_width points_map foregournd_color background_color file_name)]
   ))
