...
(define-runtime-path s20s_draw-mask-showcase_directory (build-path 'up "express" "content" "docs" "s20s_mask_showcase"))
...
(define-runtime-path index_md_file (build-path 'up "express" "content" "docs" "s20s_mask_showcase" "_index.md"))

(provide (contract-out
          [s20s-draw-mask-showcase-express (-> list? QR? void?)]))

(define (s20s-draw-mask-showcase-express mask_list mask_index qr)
  (make-directory* s20s_draw-mask-showcase_directory)
  ...
  (with-output-to-file index_md_file
    ...
    (lambda ()
      (printf "---\n")
      (printf "weight: 20s\n")
      (printf "title: \"Step20s: Mask Func\"\n")
      (printf "---\n\n")
