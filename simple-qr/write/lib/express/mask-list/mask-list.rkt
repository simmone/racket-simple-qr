...
(define (write-report-mask-list mask_list penalty_list modules express_path)
  (let* ([scrbl_dir (build-path express_path "mask-list")]
    ...
    (with-output-to-file (build-path express_path "report.scrbl") #:exists 'append
      (lambda ()
        (printf "@include-section[\"mask-list/mask-list.scrbl\"]\n\n")))
